...
3. Pick the student from the **roster dropdown** (fastest), or type/speak
   ...
   name.
4. Tap **Open camera**, frame the circled mark, tap the shutter button. The
   app detects the coloured circle and reads the number with on-device OCR,
   showing it as a live preview. Can't find it automatically? Drag to select
   the mark by hand instead.
